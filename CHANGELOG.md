...
- Importing an arf history database now preserves session, hostname, working directory, duration, exit status and metadata, not just the command and timestamp.
- Importing a history database no longer writes to the source database.
- Importing a file that is not an arf history database now reports an error instead of silently importing nothing.
