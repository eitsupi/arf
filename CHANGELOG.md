...
- `arf r-home` reports the `R_HOME` selected by arf without starting R, so external tools can use the same R version and source overrides. If R is unavailable, normal startup continues without R evaluation while `arf r-home` exits with an error because there is no path to report.
